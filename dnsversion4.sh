...
Insights
More
projectsinkhole/dnsversion3.sh
@llmututj
llmututj Create dnsversion3.sh
Latest commit 307127a 23 days ago
 History
 1 contributor
1223 lines (1072 sloc)  46.8 KB
...
PrintLogo() {
  ...
  if [ "$1" = "pico" ]; then
    ...
    CleanEcho "Sinkhole                                   Version: 1.0"
    CleanPrintf " "
  elif [ "$1" = "slim" ]; then
...
PrintPiholeInformation() {
  ...
  if [ "$1" = "pico" ]; then
    ...
    CleanEcho "${bold_text}---------------------------------------------------${reset_text}"
